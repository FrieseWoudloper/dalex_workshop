...
You can also extract the features, true outcomes and predictions.

```{r more-info-titanic, exercise=TRUE, exercise.eval=FALSE, exercise.lines=4}
head(exp_ranger$data, n = 3)
head(exp_ranger$y, n = 3)
head(exp_ranger$y_hat, n = 3)
```
...
```{r rms-titanic, exercise=TRUE, exercise.eval=FALSE}
library(rms)
set.seed(123)
model_rms <- lrm(survived ~ rcs(age)*gender + rcs(fare) + class,
                 data = titanic_imputed)
