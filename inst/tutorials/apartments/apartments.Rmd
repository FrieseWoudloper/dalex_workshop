...
description: "Learn to explore, explain and examine regression models with DALEX using the apartments dataset."
---
...
This tutorial focuses solely on explainability of regression models. However, DALEX can just as easily be used to explain classification models.
...
It also includes code examples from the excellent blog series by Anna Kozak about [Basic XAI with DALEX](https://medium.com/responsibleml/tagged/basic-xai). 
...
## 2 Apartments dataset
We will use the `apartments` dataset. It contains prices of apartments in Warsaw. 
...
An explainer object contains all elements needed to generate model explanations, for example the model, features, ground truth labels and predictions. We can also retrieve basic information about the model from an explainer.
...
set.seed(1313)
sh_rf <- predict_parts(explainer = ___, 
                       new_observation = ___, 
                       type = "shap",
                       B = 25)
plot(sh_rf, show_boxplots = FALSE)
```

```{r ex16-solution}
set.seed(1313)
sh_rf <- predict_parts(explainer = titanic_rf_exp, 
                       new_observation = johnny_d, 
                       type = "shap",
                       B = 25)
plot(sh_rf, show_boxplots = FALSE)
...
```{r q6}
question("What are practical limitations of SHAP?",
  answer("Exact computation of Shapley values takes a lot of computing time."),
  answer("It is necessary to have access to the training data or another representative dataset."),
  answer("Both statements are true.", TRUE)
)
```

### 6.3 LIME

Break-down plots and Shapley values are not well-suited for models with a very large number of explanatory variables. The reason is that all variables in the model are used to explain the prediction. If we need _sparse_ explanations <b>L</b>ocal <b>I</b>nterpretable <b>M</b>odel-agnostic <b>E</b>xplanations (LIME) are a better approach.

The first step is to generate artificial data. This is often done by using perturbations of the instance of interest. Then a glass-box model is fitted to the artificial data. The glass-box model locally approximates the more complex black-box model. The simpler model, with a smaller number of explanatory variables, makes it easier to explain predictions. 

Since the Titanic dataset is low-dimensional, we probably wouldn't use LIME in this case. LIME is usually only applied for high-dimensional data, like text and images. However, as an example, we use the LIME method to explain Johnny D's prediction for the random forest model. 

We use the `predict_surrogate` function from the `DALEXtra` package. It is a wrapper for the LIME implementation in the `localModel` package. We specify the number of artificial data points in the `size` argument of the function call.

```{r ex17, exercise = TRUE}
library(DALEXtra)
locMod_rf <- predict_surrogate(explainer = titanic_rf_exp, 
                               new_observation = johnny_d,
                               size = 1000,
                               seed = 1,
                               type = "localModel")
...
Note that this explanation is based on only three variables.

### 6.4 Ceteris-paribus profiles

Voorbeeld met continue data en categorische data.

Vergelijken van twee modellen
